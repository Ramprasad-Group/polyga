...
def chromosome_ids_to_smiles(chromosome_ids: list, chromosomes: dict,
    ...
    for i in range(1, len(mols_of_chromosomes)):
        ...
        if 'g' in L_ms_edtg and 'd' in R_ms_edtg:
            L_index_of_g = L_ms_at_idx[L_ms_edtg.index('g')]
            R_index_of_d = R_ms_at_idx[R_ms_edtg.index('d')]
            edcombo.AddBond(L_index_of_g, R_index_of_d + L_n_atoms, order=Chem.rdchem.BondType.SINGLE)
    ...
    if SMILES_connected.count('*') > 2:
        SMILES_connected = self.__longest_smiles(SMILES_connected)
